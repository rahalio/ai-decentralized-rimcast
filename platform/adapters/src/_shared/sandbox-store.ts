...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  ...
  const profile: SandboxTenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Demo Tenant' : tenantId,
    displayNameAr: tenantId === 'tnt_demo' ? 'مستأجر تجريبي' : tenantId,
  };
...
export function generateApiKeySecret(prefix = 'rimcast_demo'): string {
